...
1. REMOVE xpanded form of widget check
2. REMOVE initial size set
...
if (preferences.scalePref.value <= 20) {
	preferences.scalePref.value = 20;
}
...
function showBigMoon() {
	...
	*/
	...
	if (moonWidgetFound) {
		...
		tellWidget(preferred_form, "setPref:longitude=" + preferences.longitudePref.value);
		tellWidget(preferred_form, "setPref:latitude=" + preferences.latitudePref.value);
...
function closeLunarWidget() {
	...
	if (lunarWidgetVisible) {
		...
		if (moonWidgetFound) {
			log("Talking to the moon widget");
...
widgetHelpTimer.onTimerFired = function () {
	widgetHelp.opacity = widgetHelp.opacity - 5;
...
function checkMoonWidget() {
	...
	if (preferences.checkMoonWidgetPref.value === "enable") {
		...
		expandedForm = preferences.widgetLocationPref.value;
		...
		if (expandedForm !== "") {
			...
			if (filesystem.itemExists(expandedForm)) {
				log("%TIMEK-I-CHKM, 3 Checking expanded form of the Moon widget - " + filesystem.itemExists(expandedForm));
				preferred_form = expandedForm;
			} else if (filesystem.itemExists(widgetForm)) {
				log("%TIMEK-I-CHKM, 4 Checking widget form of the Moon widget - " + filesystem.itemExists(widgetForm));
		...
		if (filesystem.itemExists(expandedForm) || filesystem.itemExists(widgetForm)) {
			log("%TIMEK-I-CHKM, 5 Moon widget III is installed." + preferred_form);
...
function checkUnderWidget() {
	...
	if (preferences.underWidgetPref.value === "enable") {
		...
		//expanded form
		expandedForm = preferences.underwidgetLocationPref.value;
		...
		if (expandedForm !== "") {
			...
			under_preferred_form = widgetForm;
			log("under_preferred_form ", under_preferred_form);
		}
...
widget.onload = function () {
	...
	if (underWidgetFound === true) {
		...
		tellWidget(under_preferred_form, "setPref:level=\"below\"");
	}
...
pin.onMouseDown = function () {
	if (main_window.locked) {
		...
		if (moonWidgetFound) {
			//tellWidget(preferred_form, "setPref:locked=false");
		}		
		pin.opacity = 0;
...
timekeeperclickpointleft.onClick = function () {
	...
	if (timekeeperPos === 0) {
		...
		r = new RotateAnimation(monthCog, 0, animationSpeed, animator.kEaseOut);
		if (clockPos === 0) {
			clockclickpointright.onClick();
		}
	} else {
		...
		p = new MoveAnimation(timekeeperclickpointfarright, timekeeperclickpointfarright.hoffset - (50 * scale), timekeeperclickpointfarright.voffset, animationSpeed, animator.kEaseOut);
		q = new MoveAnimation(timekeeperclickpointbottom, timekeeperclickpointbottom.hoffset - (50 * scale), timekeeperclickpointbottom.voffset, animationSpeed, animator.kEaseOut);
		r = new RotateAnimation(monthCog, 180, animationSpeed, animator.kEaseOut);
		if (clockPos === 1) {
			clockclickpointright.onClick();
		}
	}
	animator.start([
		a, b, c, q, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r
	]);
};
...
function donate() {
	...
	if (answer === 1) {
		openURL("https://www.paypal.com/uk/cgi-bin/webscr?cmd=_flow&SESSION=8_BdXSZ5coA0xZ1vTaYsjrqUoQqJ6KSjqczG3DRnT6xbRX1thxa2TBoRXLO&dispatch=50a222a57771920b6a3d7b606239e4d529b525e0b7e69bf0224adecfb0124e9b61f737ba21b081981b5bc1bed9bcd4a664bb0e309d0a9e4f");
